        ...
        settings = {
          vim = {
            ...
            extraPlugins = with pkgs.vimPlugins; {
              snacks = {
                ...
                setup = ''
                  require("snacks").setup({
                    ...
                    notifier = { enabled = true }, indent = { enabled = true }, scroll = { enabled = true },
                    bigfile = { enabled = true }, quickfile = { enabled = true }, statuscolumn = { enabled = true },
                    words = { enabled = true }, terminal = { enabled = true }, input = { enabled = true },
                    lazygit = { enabled = true }, picker = { enabled = true },
                  })
                '';
              };
              trouble = {
              ...
              yazi = {
                package = yazi-nvim;
                setup = ''pcall(function() require("yazi").setup({ open_for_directories = true, change_neovim_cwd_on_close = true, floating_window_scaling_factor = 0.75 }) end)'';
              };
            };
            ...
            luaConfigPost = ''
              ...
              local function apply_theme()
                ...
                hl(0, "GitSignsAdd", { fg = colors.success, bg = "NONE" }); hl(0, "GitSignsChange", { fg = colors.warning, bg = "NONE" }); hl(0, "GitSignsDelete", { fg = colors.error, bg = "NONE" })
              ...
              map("n", "<leader>ff", function() Snacks.picker.files() end, { desc = "Find Files" }); map("n", "<leader>fg", function() Snacks.picker.grep() end, { desc = "Live Grep" }); map("n", "<leader>fb", function() Snacks.picker.buffers() end, { desc = "Find Buffers" }); map("n", "<leader>fh", function() Snacks.picker.help() end, { desc = "Find Help" }); map("n", "<leader>fs", function() Snacks.picker.smart() end, { desc = "Smart Find Files" }); map("n", "<leader>fw", function() Snacks.picker.grep_word() end, { desc = "Search Word" })
              map("n", "<leader>th", function() Snacks.terminal.toggle(nil, { win = { position = "bottom" } }) end, { desc = "Toggle Horizontal Terminal" }); map("n", "<leader>tv", function() Snacks.terminal.toggle(nil, { win = { position = "right" } }) end, { desc = "Toggle Vertical Terminal" }); map("n", "<C-a>", function() Snacks.terminal.toggle("opencode", { win = { position = "right" } }) end, { desc = "Toggle Opencode AI" })
              map("n", "<leader>gg", function() Snacks.lazygit.open() end, { desc = "Open LazyGit" }); map("n", "<leader>gb", function() Snacks.picker.git_branches() end, { desc = "Git Branches" }); map("n", "<leader>gs", function() Snacks.picker.git_status() end, { desc = "Git Status" })
              map("n", "<leader>e", function() require("yazi").yazi() end, { desc = "Open Yazi" })
              map("n", "]d", function() vim.diagnostic.jump({ count = 1, float = true }) end, { desc = "Next Diagnostic" }); map("n", "[d", function() vim.diagnostic.jump({ count = -1, float = true }) end, { desc = "Prev Diagnostic" }); map("n", "<leader>xx", "<cmd>Trouble diagnostics toggle<cr>", { desc = "Diagnostics (Trouble)" })
              vim.api.nvim_create_autocmd("TextYankPost", { callback = function() vim.highlight.on_yank() end })
              vim.api.nvim_create_autocmd("BufReadPost", { callback = function() local mark = vim.api.nvim_buf_get_mark(0, '\"'); local lcount = vim.api.nvim_buf_line_count(0); if mark[1] > 0 and mark[1] <= lcount then pcall(vim.api.nvim_win_set_cursor, 0, mark) end end })
              local function transparent_bg() local groups = { "Normal", "NormalFloat", "SignColumn", "EndOfBuffer", "MsgArea", "SnacksDashboardNormal", "SnacksPickerNormal", "SnacksPickerList" }; for _, group in ipairs(groups) do vim.api.nvim_set_hl(0, group, { bg = "NONE", ctermbg = "NONE" }) end end
              vim.api.nvim_create_autocmd({ "ColorScheme", "VimEnter" }, { callback = transparent_bg })
              vim.api.nvim_create_autocmd("FileType", { pattern = "markdown", callback = function() vim.opt_local.wrap = true; vim.opt_local.linebreak = true; vim.opt_local.breakindent = true end })
            '';
          };
        };
      };

      # Custom Desktop Entry for Neovim (to use user terminal)

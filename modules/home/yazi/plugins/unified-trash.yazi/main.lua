...
function M.obtain()
	local output, err = Command("sh"):arg({ "-c", "trash-list" }):output()
...
function M:entry(job)
	...
	while true do
		...
		if run == "quit" then break
		elseif run == "up" then update_cursor(-1)
		elseif run == "down" then update_cursor(1)
		elseif run == "restore" then
			local item = get_active_item()
			if item then
				-- trash-restore is interactive: shows matches and waits for a number.
				-- Pipe "0" to auto-select the first (and usually only) match.
